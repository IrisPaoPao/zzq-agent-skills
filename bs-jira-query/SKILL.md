...
用户明确要求写备注或“修改状态并备注”时，先执行 `issue show` 核对上下文，再准备备注。仅要求“标记开发完成”只更新状态，不附带评论。备注不得只写“已修复”或只放提交号，至少覆盖以下六类信息；不适用的项目要明确写“无”：
...
2. 使用列表中的精确状态名执行 `bsq-jira issue transition <KEY> --to "开发完成"`；仅在用户同时明确要求备注时追加 `--comment "<完整处理备注>"`。状态以列表返回值为准。
3. 再执行 `bsq-jira issue show <KEY>` 复核状态；本次写了备注时另用 `comment list` 复核备注，分别报告结果。
...
2. **安全、高效地获取附件资源**：在读取 `issue show` 的输出时，注意观察“附件”区块。
   ...
   - **唯一推荐做法**：直接使用原生下载命令：`bsq-jira attachment download '<附件URL>' --dest-dir ./`，它会自动处理 Cookie 与中文文件名；下载目录遵循工作区产物约定。越界文件名或同名文件会报错，中断下载不保留半成品；遇到冲突换明确的保存目录，不删除旧文件。
