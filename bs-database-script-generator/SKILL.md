...
3. **确认写入位置并决定文件集合。** 仅在产品 `temp/` 目录新建或修改未归档草稿。`backup/` 和 `business/` 中的脚本只可用于核对历史与学习风格，绝不能修改。若已归档脚本需要修复、补充或纠正，必须在 `temp/` 新增一对后续迁移脚本来达到目标状态。默认 TDSQL 和 Oracle 各一份。多张表按表拆；多类操作（建表 vs 改表 vs 加索引）按操作类型拆；同一张表的多字段、多索引可以合并到一个文件，**但每个字段、每个索引各自一个 `if` 包裹**（原因见硬约束 #1）。
...
- **写入边界：** 仅向 `temp/<product>/` 写入新的迁移脚本。`backup/`、`business/` 和任何已发布迁移目录都是只读历史，不因修复需求而 patch；以新增脚本表达后续修复。
- **命名格式：**
  ```
...
| `nullable(table, column)` | 字段允许 NULL → true | 仅用于查询元数据；不要作为 `ALTER TABLE ... MODIFY` 的执行条件 |
