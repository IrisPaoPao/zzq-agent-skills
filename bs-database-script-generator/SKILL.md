...
2. **明确输入。** 与用户确认：产品（目录）、数据库（默认 TDSQL+Oracle 双版本）、版本号（缺省从 business 目录的最新版本推断）、作者。作者风格不写死——按下文「作者风格识别」实时去仓库读该作者最近的脚本来对齐注释格式、序号位数、字段类型偏好。
...
每个作者在仓库里都积累了自己的风格——头注释格式、序号位数、`notExist` 用 `select *` 还是 `select 1`、Oracle 字段是 `VARCHAR2(N)` 还是 `VARCHAR2(N CHAR)`、表空间偏好。这些不要靠 skill 写死，**进入流程后实时去仓库里学**：

1. 确认作者名（用户给定，或从 `git config user.name` 推断）。
2. 在 `行业应用/backup/` 与 `行业应用/business/<product>/<version>/` 下用 `find ... -name "*<作者>*"` 找该作者最近 3–5 个脚本（按日期倒序）。
3. **直接读其中 1–2 个**，提取以下要素并复用到本次生成中：
   - 头注释格式（双行 `// jira描述` + `// 姓名` vs JavaDoc `/** @author @date @describe */`）
   - 序号位数（一位 `1` vs 两位 `01`）
   - `notExist` 写法（`select *` vs `select 1`）
   - Oracle `VARCHAR2` 修饰（`VARCHAR2(64)` vs `VARCHAR2(64 CHAR)`）
   - 表空间偏好（基础表 vs 业务表的实际选择）
   - 即使提供 MySQL SQL 是否产出 MySQL 文件
4. 找不到该作者历史脚本时，按基线规范走：双行注释 `// jira-id 描述` + `// 姓名`，类型与表空间沿用相邻产品脚本。

仓库里**一定不要把任何具名作者的偏好写进 skill 文件**——既会泄露团队信息，也会随成员流动失效。

## 产品规则识别（动态读相邻脚本，不写死）

类似地，每个产品（`temp/<product>/`）的表空间约定、表前缀习惯、分表方式（按年/按月/HASH/不分表），都从该产品 `business/<product>/<最新版本>/` 与 `backup/<product>/<最近日期>/` 的脚本里学：

1. 找该产品最近 3–5 个建表脚本看 Oracle 表空间名、TDSQL 是否分表、`shardkey` 字段、`PARTITION` 风格。
2. 找该产品最近的 alter 脚本看字段类型偏好、注释风格。
3. 表前缀直接看该产品建表脚本的统一前缀（如 `xxx_*`、`auth_*`），需要新增表时沿用。
4. 分表表的字段名、索引名要避开全表名（忽略大小写），给 `_${monthSuffix}` / `_${yearSuffix}` 替换留位置。

具体某个产品有什么"特色规则"，**用到时再去读**，不预设。
