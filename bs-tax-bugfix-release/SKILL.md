...
先核对本次涉及的仓库、来源基线、目标分支、上游分支、工作区改动和已有 worktree。缺少影响发布线的输入时询问，不自行决定。提交、推送及跨仓库发布只在用户明确授权的范围内执行；不自动 stash、提交现场改动或切换主工作区分支。
...
2. **准备分支**——
   - **用户指定既有 SNAPSHOT 分支时，不要新建分支**：不要从 tag 新拉分支；在隔离 worktree 中检出该已有分支（`git worktree add ../<wt-dir> <target-branch>`），进入后检查分支、上游和工作区状态，再 `git pull --ff-only`；缺上游或拉取失败就停止询问。
   - **用户未指定现成发布分支时**，才从目标 tag 拉新分支：先 `git branch -a | grep <new-version>` 查远程是否已有同名分支；若已有，看它的版本号和改动是否就绪（可能是别人建好版本号但没修 bug），避免重复创建。没有则在用户指定的基线和任务分支上创建隔离 worktree：`git worktree add -b <new-branch> <wt-dir> <tag>`。新分支若没有上游，明确报告并询问后续提交/同步方式，不绕过提交前拉取要求。
...
5. **精确提交**——提交前完成本机验证，再检查分支、上游和工作区，执行 `git pull --ff-only`；缺上游、拉取失败或需要 merge/rebase 时停止询问。按具体文件或补丁精确暂存并检查暂存区，不能带入用户已有修改；已有无关暂存内容时先询问处理方式，不自动取消暂存。提交信息使用中文并遵循项目 Jira 关联规范，提交后用 `git show --stat HEAD` 核对范围。
6. **打 tag + push**——**先按硬约束 #4/#5/#6 检查所有 pom 无 SNAPSHOT、tag 在 Git 与 Maven 均未占用且 revision 与 tag 一致、并完成必要构建验证**，再 `git tag -a <version> -m 'release <version>: ...'`，`git push origin <version>`。仅当依赖仓库本次确有修改且属于用户已授权发布范围时，按依赖顺序单独校验、构建和发布。未修改依赖只核对已发布版本，不额外打 tag。分支推送与 tag 推送分别核对目标远端和授权范围，不能把 tag 推送当作分支已同步。
...
1. **保护已有改动。** 先检查各 worktree 的分支与状态，使用明确基线的隔离工作区；不自动 stash、提交无关文件或强制切换。发现已有改动与目标任务重叠时，保留现场并说明冲突。
2. **提交前检查实际暂存内容。** 精确暂存任务文件；同一文件混有无关修改时按补丁暂存。`git commit <pathspec>` 会读取指定文件的工作区内容，不能用它保证只提交已经核对的暂存片段。
3. **发版前查 maven 仓库确认版本号未占用。** 本地 maven 仓库可能在非默认路径（不一定是 `~/.m2`，常见 `worksoft/mavenRepo`），先看 `~/.m2/settings.xml` 的 localRepository 和实际发布仓库配置，脱敏核对远端制品是否存在；本地缓存缺失不能证明远端未占用。已发布的正式版 maven 不允许覆盖，版本号撞了必须换。
...
5. **打 tag 前必须完成 tag / 版本一致性硬校验。** 必须同时满足：
   ...
   - 待创建 tag 在 **本地 Git、目标远端 Git 和 Maven 发布仓库中都不存在**（检查本地 tag、用 `git ls-remote --tags <remote> refs/tags/<version>` 检查远端，并按硬约束 #3 查 Maven）；网络失败属于未确认，不等于不存在。
...
7. **工作区可能有其他分支带过来的改动。** 切分支后先 `git status` 看清哪些是本次发版要提交的、哪些是别的工作的，仅将前者纳入经检查的暂存区。
...
# 此时先核对已有目录的改动与用户指定工作区；不可自动改主工作区，冲突时询问。
...
git worktree add -b <new-branch> <wt-dir> <tag>
...
git add -- <file1> <file2>
git diff --cached
git commit -m 'fix: [JIRA-KEY] 修复说明'
git show --stat HEAD
